...
def _require_admin(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """要求管理员角色"""
...
def save_data_source(body: DataSourceConfig, _user: User = Depends(_require_admin)):
...
def get_oracle_config(_user: User = Depends(_require_admin)):
...
def save_oracle_config(body: OracleConfig, _user: User = Depends(_require_admin)):
...
def test_oracle(_user: User = Depends(_require_admin)):
    cfg = load_config().get("oracle", {})
    try:
...
def oracle_query_test(body: dict, _user: User = Depends(_require_admin)):
...
def get_postgresql_config(_user: User = Depends(_require_admin)):
...
def save_postgresql_config(body: PostgreSQLConfig, _user: User = Depends(_require_admin)):
...
def test_postgresql(_user: User = Depends(_require_admin)):
    cfg = load_config().get("postgresql", {})
    try:
        cfg["password"] = decrypt_value(cfg.get("password_enc", ""))
    except Exception:
...
def postgresql_query_test(body: dict, _user: User = Depends(_require_admin)):
...
def get_dify_config(_user: User = Depends(_require_admin)):
...
def save_dify_config(body: DifyConfig, _user: User = Depends(_require_admin)):
...
def test_dify(_user: User = Depends(_require_admin)):
    cfg = load_config().get("dify", {})
    try:
        key = decrypt_value(cfg.get("api_key_enc", ""))
...
def debug_dify(body: DifyDebugRequest, _user: User = Depends(_require_admin)):
...
def save_departments(body: DeptConfig, _user: User = Depends(_require_admin)):
...
def get_scheduler_config(_user: User = Depends(_require_admin)):
...
def save_scheduler_config(body: SchedulerConfig, _user: User = Depends(_require_admin)):
...
def get_push_settings(_user: User = Depends(_require_admin)):
    cfg = load_config().get("push", {})
    return PushSettings(**cfg)


@router.post("/push", response_model=MessageResponse, summary="保存推送参数")
def save_push_settings(body: PushSettings, _user: User = Depends(_require_admin)):
...
def get_notify_config(_user: User = Depends(_require_admin)):
    cfg = load_config().get("notify", {})
    return NotifyConfig(**cfg)


@router.post("/notify", response_model=MessageResponse, summary="保存通知渠道配置")
def save_notify_config(body: NotifyConfig, _user: User = Depends(_require_admin)):

"""
Static checks for the phase-3 patient QC and relay-alert UI.
...
def test_relay_alert_tab_contains_phase3_workbench_sections():
...
def test_patient_qc_js_uses_existing_relay_alert_apis():
    ...
    assert "relayAlertStatusLabel(status)" in js
    assert "relayAlertSeverityLabel(severity)" in js
...
def test_patient_qc_phase3_assets_are_cache_busted():
    html = _read(_INDEX_HTML)
    js = _read(_APP_JS)

    assert "/styles/pages/patient_qc.css?v=20260608-patient-qc-phase3" in html
    assert "/templates/pages/patient_qc.html?v=20260608-patient-qc-phase3" in html
    assert "/scripts/app.js?v=20260608-patient-qc-phase3" in html
    assert "./modules/patient_qc.js?v=20260608-patient-qc-phase3" in js

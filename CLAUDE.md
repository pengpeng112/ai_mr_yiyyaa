...
按患者分组的病历+护理数据
    ↓ services/payload_builder.py（结构化 Dify 入参）
...
| `app/services/payload_builder.py` | 组装结构化 Dify 入参（`build_dify_payload` / `build_dify_mr_text`），支持中英文字段别名映射 |
...
运行时配置存储在 `config/config.json`，通过 `/api/config` 接口读写，密码字段以 `password_enc`/`api_key_enc` 加密存储。

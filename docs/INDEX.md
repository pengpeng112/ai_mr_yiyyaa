...
| 030 | `ACTIVE/030_PREARCHIVE_RULE_MAPPING_TABLE_V1_UNSIGNED_20260828.md` | **评分项-规则映射表 v1.1（七问已拍板+扣分频次TOP实证；正式签字栏仍待质控科）**（2026-08-28） | CDMS.T_MARK_ITEM 全量 92 条逐条分类：A 类 23（一期可落地 11/阻塞 8/二期 4）、B 类 7（并入六类语义）、C 类 62（提示级/不做）；含数据源实测依据、参数草案、严重度建议、签字栏与质控科七项重点裁决问题。 |
...
| 033 | `ACTIVE/033_PENDING_REQUESTS_QC_IT_USER_20260829.md` | **三项待办请求书（2026-08-29，可转发）：①致质控科——030 v1.1 92条映射表正式签字+版本号YYYY.MM.DD-qc-vN（W2）②致信息科——病理SQLServer驱动补装+血透/电测听/气管镜/基本信息(HIS@10.10.10.14)四源登记进数据资产平台（顺带核实心电登记）③致业务负责人——W10系统推送类规则清单5字段模板（病理+PACS两条起步）** | 031/032 交付后外部依赖收口：每件事写明致谁/请什么/具体IP库名视图名/需给回的格式与模板/到位后解锁什么；三件互相独立可并行。 |

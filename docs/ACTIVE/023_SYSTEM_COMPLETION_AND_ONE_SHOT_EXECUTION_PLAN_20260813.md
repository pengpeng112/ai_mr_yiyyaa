...
| --- | --- | --- | --- | --- | --- |
| 048 本地完善专项 | 系统矩阵、工作台分页与双前端、任务恢复、专项及全量验收 | 计划已编制，待执行（2026-09-15） | 048 v1.0；049预留；PROMPT-20260915 | 046/047已登记本地交付，执行时重核现场 | 本次仅计划文档；后续按执行请求限定本地范围，零生产/Git写授权 |
| 054 本地模拟验收专项 | 全系统68项清单与U0–U7修复验收 | **已执行完毕（2026-09-23）**：U0–U7 全包；4 缺陷修复（权限 fail-closed/质控记录序号守卫/mock 高危达标/锚 schema 陈旧断言）；编排终轮 7 套件 19 执行 0 失败；full 门禁 11/11 PASS 锚全不低于（unit 69>锚 62）；U5 由并行会话分工完成；J09–J12 与生产两批仍按 §2/§3 前置推进 | 054 §8–§14＋**§15（交付）**；orchestrator/gate-runs/l1/j_offline 证据指针见 §15.4 | 049/051已完成能力回归，当前现场重核 | 本次仅文档；生产两批仍按054§2/§3既有批准和前置，不新增外发/Git授权 |
...
| 041 隔离可用性（2026-09-04 追加） | 039 规则中心隔离 demo 可点可用（sidecar/BFF/权限双修/四角色矩阵/双前端 E2E）；从属本文件，不构成新执行入口 | **已执行完毕（2026-09-04，交付=042）**；零生产写入、未 commit | `docs/ACTIVE/041_*` + `docs/ACTIVE/042_*` + `开发起步包/PROMPT-20260904_规则中心隔离可用性一次性执行.md` | 乙类现场项仍服从 WP5/WP6/WP7/WP11 与 036-RP7 | 无生产写入 |
| 043 跨会话效率治理（2026-09-07 追加） | 3 Skill（gates/oneshot-delivery/demo-stack）+3 脚本（run_gates/clean_demo_ports/update_gate_anchor）+首版 `docs/reference/gate_anchors.json`+指令治理 C1-C5（只指针化不删除）+卡点台账+conftest env 守卫+全量回归重跑（R1-R5）；从属本文件，不构成新执行入口 | **已执行完毕（2026-09-07，交付=044）**；零生产写入、未 commit、未创建定时任务（cron 仅建议清单） | `docs/ACTIVE/043_*` v1.1 + `docs/ACTIVE/044_*` + `开发起步包/PROMPT-20260906_跨会话效率治理一次性执行.md` | 历史文档带日期数字未改写；业务代码零改动 | 无生产写入 |
| 045 044 核查修复（2026-09-08 追加） | 核查 044 后的修补：run_gates PASS 耗时累加 + sidecar 拉起失败杀树 + 044 文末 §11 更正 + 09-08 指令修订 01 补登记；从属本文件，不构成新执行入口 | **已执行（合并046完成，2026-09-11）**：F0–F5/D1–D8 全核销，更正落 044 §11，证据=047 §10/§11；零生产写入、未上抬锚、未创建定时任务 | `docs/ACTIVE/045_*` v1.0 + `开发起步包/PROMPT-20260908_044核查修复一次性执行.md` | 不改写 044 §0–§10；不回滚 09-08 指令修订 | 无生产写入 |
| 046 无纸化规则与JHEMR闭环（2026-09-10追加） | 92项覆盖/AI自动匹配与trial观察、预检运行断点修复、提交提醒接口/核查整改/复检投递；保留原AI；从属本文件 | **已执行完毕，交付=047（2026-09-11，两段执行）**：T0-T10 全包闭合、终版门禁全绿含 legacy 双轮实跑；用户已确认提交时提醒、不强制阻断，JHEMR 客户端另行安排；L1 已验收，L2/L3 与生产部署未开始；零生产、未 commit | `docs/ACTIVE/046_PAPERLESS_RULES_JHEMR_CLOSED_LOOP_PLAN_20260910.md` v1.2；配套PROMPT-20260910；交付预留047 | T0–T10；真实JHEMR接口双方联调与试点按046 §8执行；045全部F0–F5/D1–D8及09-09处置纳入046，按子包拓扑顺序实施，必要时T5后两段 | 本轮只授权调查与计划，无生产写入 |
| 048 系统可靠性与工作台完善（2026-09-15追加） | 046 后本地完善批：工作台分页/真实total/有界聚合/科室范围强制（详情+动作）、双前端错误恢复+权限按钮+中文状态、匹配任务事务健壮性、工作台+JHEMR L1 真后端专项、门禁双 spec；从属本文件 | **已执行完毕，交付=049（2026-09-15）**：T0-T7 全包；发现并修复 BFF issue 动作 any-of 缺陷；发现 D-J1（JHEMR GET 终态后 issues 毫秒级瞬态窗口，trigger.py 不在白名单未修，列 049 §10）；047 两处事实更正（§13）；零生产、未 commit | `docs/ACTIVE/048_*` v1.0 + `docs/ACTIVE/049_*` + `开发起步包/PROMPT-20260915_系统可靠性与工作台完善一次性执行.md` | 全量门禁+专项结果以 049 §6 为准 | 无生产写入 |
| 050 D-J1 修复与历史批次分批提交（2026-09-21 追加） | D-J1 终态物化顺序修复（trigger.py 物化前移+失败降级 partial）+043–049 未提交累积终态五批本地 commit（B1–B4=140 文件+B5 本批）+门禁锚更新；round-6 三方互查修订后执行，commit 条款经用户选项 A 批准（不 push）；从属本文件 | **已执行完毕，交付=051（2026-09-21）**：T0–T5 全包；全量门禁 11/11 锚全不低于（main 1295→1379/prearchive 283→455）；L1 ALL PASS first==stable==expected=6；附带修复 L1 预检栈 :memory: 共享连接事务互吞并更正 049「稳定态 4 条」伪影；零生产、未 push | `docs/ACTIVE/050_*` v1.1 + `docs/ACTIVE/051_*` + `开发起步包/PROMPT-20260921_*.md` | 全量门禁与五批 hash 以 051 §2/§4 为准 | 无生产写入 |
| 052 JHEMR 客户端病历完成提醒调查（2026-09-21 追加） | 028 场景/027 补路径：客户端「病历完成」点击质控提醒**接线调查+条件性待批实施包**（T4 生产实施/T5 AI 闭环均另批）；round-7 三方互查修订后执行；从属本文件 | **已执行完毕，交付=053（2026-09-22）**：T1=**甲分支**（宿主 `widgetdll/JHInPatMedical.dll` 的 returnFinishbyEmrFileConfig 已调患者级 CustomEmrFileVerifyConfig(…,4,…)；btnMRCom「病历完成」/btnMrCommit 两入口覆盖；TIP_TYPE=1=提示不阻断通道成立）；T2 三表实证+4 条候选规则演练命中/不命中对照通过（177 副本只读）；T3 就绪包 dry-run 落 review/jhemr-l2-20260921/；证伪 v1.1「4/7 无调用点」锚点（真宿主不在制定者七目录）；零生产写入、客户端零修改、E:\嘉和 零改动、未 commit | `docs/ACTIVE/052_*` v1.1 + `docs/ACTIVE/053_*` + `开发起步包/PROMPT-20260921_JHEMR*.md` | T4 试点实施=用户当次逐项批准；生产 INSERT/激活/回滚脚本以 053 §4 就绪包为准 | 无生产写入（本批只读反编译+177 只读 SELECT） |
...
> 2026-09-08 授权范围澄清（用户批准指令治理修订）：本节 APPROVED/NOT_APPROVED 记录原任务授权状态，不构成后续任务的常设授权。当前任务已覆盖具体动作、范围、条件和所需证据的授权无需重复请求，可由多条书面指令组成；生产、临床、外发及精确写入审批要求不变。Git commit/tag/push/PR 分别服从当前任务明确授权和 00_AI协作规则 §6；下表历史本地提交批准不能自动用于新任务。

...
最高优先级规则：
1. 任一必需文书整体缺失、为空或只有占位内容：conclusion="manual_review"、issues=[]；不得输出明确问题或 high。
2. 一份有记录、另一份未提及不等于冲突；不得因缺项直接 high。
3. evidence 的 source 必须真实：手术记录只能标 operation_record，绝不能标成 preop_record 或 postop_record。
4. 不评价手术方案合理性，不推测未记录的过程。
...
规则：
1. 任一必需文书缺失：全部维度 unknown/low/gray、confidence<=0.59，has_inconsistency=false，写 manual_review，严禁 high。
2. high 仅允许同一 contradiction issue 满足 severe+high_eligible+不同真实 source+双方证据非空+confidence>=0.8+受控 safety_category。
3. source_a/source_b 相同或任一为空时不得 high。
4. 明确 general→warn/medium/yellow；明确 omission 最高 medium；hint→warn/low/blue；真正无法确认→unknown/gray。
5. 为兼容现有解析器，将第一来源证据放 medical_evidence、第二来源放 nursing_evidence；真实 source_a/source_b 必须完整保存在 extra.issues，页面不得把它们统一解释成病程/护理。
...
最终自检：缺文书、单侧未提及、术后一般缺项、术前正常“拟行手术”和伪 source 均不得 high。只输出 JSON。
```

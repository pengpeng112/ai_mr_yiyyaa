...
description: 安全核查 Med-Audit 历史高危质控结果，并通过脱敏离线包引入外部 AI 复核；经人工批准后分批降级不符合高危硬门槛的记录，同时按 patient_id + visit_number（业务视图“患者ID”+“次数”）补齐缺失的在院/出院科室信息。适用于生产服务器、Oracle 应用库、JHEMR.V_QYBR 业务视图、PushLog/AuditDimensionResult/AuditConclusion 历史整改。复核或整改历史患者高危/红色质控结果、回填历史科室数据及其外部 AI 复核时必须使用；普通代码、文档、技能审阅和无关生产数据库任务不触发。
---
...
按本任务所需阶段执行，每阶段输出报告。到达明确审批点时，核对当前任务中已取得的书面授权；若已覆盖该阶段、对象和前置证据则继续，无需授权集中在同一条消息。仅科室回填任务执行适用的阶段0、阶段5和阶段6，不要求先做高危整改。候选范围与脱敏方案、复核包外发、临床目标等级、精确写入批次及生产回滚的审批点全部保留；尚未生成的精确包或哈希不能由泛化授权代替批准。等待批准期间可继续不依赖该批准的已授权工作。

### 阶段 0：本地与生产只读预检

1. 检查 `git status`，不得清理、覆盖或回滚他人改动。
2. 确认应用库类型、表前缀、容器、镜像、单 worker、健康状态和当前配置；不得打印密钥。
   - 容器不是 `healthy`、健康接口超时或 worker 数无法确认时，记录基线后暂停依赖该健康状态的生产写入与后续批次，不得进入写入阶段，不得猜测或绕过；可继续不会增加故障负担的受限只读诊断、本地脚本检查和报告整理，无法安全继续的部分明确列为阻塞。
...
停止点：高危整改结果确认是进入科室生产写入的前置条件。等待确认期间可继续已授权的科室只读分析和 dry-run；科室回填写入完成后立即完成只读对账、报告和隔离回滚演练（阶段 6），再连同结果与冲突清单一并提交用户确认；确认不作为这些验收工作的前置条件。
...
停止点：科室回填写入前需用户批准回填清单；写入完成后立即执行阶段 6 只读对账、报告与隔离回滚演练，产出结果与冲突清单提交用户确认（确认不是阶段 6 的前置条件）。
...
任何不确定医学判断标记“需临床专家确认”；任何数据库结构或字段不一致：暂停依赖该结构或字段假设的生产写入及后续批次，不得猜测或绕过，可继续不依赖该假设的只读诊断、本地脚本检查和报告整理，并将阻塞项明确列入报告。
